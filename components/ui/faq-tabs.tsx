...
const FAQHeader = ({ title, subtitle }: { title: string; subtitle: string }) => (
  <div className="relative z-10 flex flex-col items-center justify-center mb-8">
    <span className="label mb-4" style={{ color: 'var(--muted-foreground)' }}>{subtitle}</span>
    ...
    {Object.entries(categories).map(([key, label]) => (
      <button
        ...
        style={{
          padding: '8px 20px',
          borderRadius: '100px',
          border: `1px solid ${selected === key ? 'var(--foreground)' : 'var(--border)'}`,
          background: selected === key ? 'var(--foreground)' : 'transparent',
          color: selected === key ? 'var(--background)' : 'var(--muted-foreground)',
          fontSize: '0.875rem',
          fontWeight: 500,
          cursor: 'pointer',
          transition: 'all 0.2s',
